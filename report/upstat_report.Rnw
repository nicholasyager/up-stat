...
\begin{figure}[h]
\centering
<<LOCplot,echo=FALSE, fig=TRUE, height=3.5>>=
...
ggplot(LOSFrame, aes(x=LOS, y=Delay)) +
...
We also examined the relationship between the traffic density (vehicles/mile)
and traffic volume (vehicles/hour), also known as flux. Whe plotted, the
relationship between the traffic density and the traffic volume creates the
fundamental diagram of traffic flow \cite{HCM}, as shown in Figure \ref{fig:Fundamental}.
The fundamental diagram presents a curve in which points on the left of the line
represent free and bound traffic flow, and points on the right of the line
represent the velocity of congested flow. Using linear regression techniques, we
...
more unstable leading to traffic waves and eventually congestion. To maximize
the traffic flow, the vehicle density must remain below the critical density.

\begin{figure}[h]
\centering
<<echo=FALSE, fig=TRUE, height=3.5, results=hide>>=
...
@
\caption{\leftskip0.5in\rightskip0.5inFundamental diagram of traffic flow provides
...
\begin{figure}[h]
\centering
<<Binning,echo=FALSE, fig=FALSE, cache=TRUE, eval=TRUE, height=3.5>>=
...
@
\caption{\leftskip0.5in\rightskip0.5in The median traffic delay and median weekly
