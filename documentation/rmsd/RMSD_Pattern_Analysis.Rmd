...
accept two vectors of equal length, and it must also be able to tolerate NA v
values in the data.
```{r, echo=TRUE}
...
other huristic methods, but it does not fall prey to missing data problems that
...
The `rmsds` matrix returned can be visualized either as a timeseries, which is a
...
It is important to note that one abnormal day will be noticed asa high RMSD as 
